...
class Meta():
    ...
    def build_metalearner_graph(self):
        ...
        if self.args.net == 'ConvNet':
            from learners.convnet import ConvNet
            net = ConvNet(self.args)
        elif self.args.net == 'ConvNetBN':
            from learners.convnet import ConvNetBN
            net = ConvNetBN(self.args)
        elif self.args.net in ['ResNet']:
            from learners.resnet import ResNet
        ...
        elif self.args.net in ['VGG11', 'VGG13', 'VGG16', 'VGG11BN', 'VGG13BN', 'VGG16BN']:
            from learners import vgg as vgg_module
            net = getattr(vgg_module, self.args.net)(self.args)
                               ...
                               'InceptionResNetV2', 'InceptionV3', 'VGG19', 'Xception',
                               'DenseNet40', 'MobileNetV2', 'MobileNet']:
            from learners.general_model import KerasModel
    ...
    def global_initialize(self, args, sess):
        ...
        if args.pretrain:
            ...
            weightapi = build_tracking_api(args)
            pretrain_weights = comet_pull_weight_by_key(key, projname, 200, weightapi, 'allranks')
