...
def craft():
    ...
    experiment = Dummy()
    if rank == 0:
        experiment = LocalExperiment(project_name=args.craftproj, root=args.artifactroot, workspace=args.workspace)
...
def train(nepoch):
    ...
    expt = Dummy()
    if args.weightset != '':
        expt = LocalExperiment(project_name=f'weightset-{args.net}-{args.weightset}'.lower(), root=args.artifactroot, workspace=args.workspace)
