...
def comet_pull_weight(epoch, api, args, rank, deterministic=True):
    for attempt in range(4):
        try:
            tic = time()
            projname = f'{args.workspace}/weightset-{args.net}-{args.weightset}'.lower()
...
def comet_pull_weight_by_key(key, projname, epoch, api, rank, deterministic=True):
    for attempt in range(4):
        try:
            tic = time()
            expt = api.get(args_workspace_from_api(api), projname, key)
            assets = expt.get_asset_list()
...
def build_tracking_api(args):
    return LocalAPI(root=args.artifactroot, workspace=args.workspace)


def args_workspace_from_api(api):
...
def uid2craftkey(uid, craftproj):
    raise RuntimeError('uid2craftkey(uid, craftproj) now requires args; use uid2craftkey_with_args instead.')


def uid2craftkey_with_args(uid, craftproj, args):
    api = build_tracking_api(args)
    expts = api.find_by_uid(craftproj, uid)
    ...
    if len(expts) > 1:
        print(f'there were {len(expts)} expts with same uid. taking most recent one at {expt.url} '
